            ...
            try
            {
                ...
                if (MessageBox.Show("確定要刪除帳號嗎?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (deleEventCnt != 0)
                    {
                        ...
                        SqlCommand sqlCommandDeleteStatus = new SqlCommand("delete [LogInStatus] where [ManagerID]=@ID", conn);
                        sqlCommandDeleteStatus.Parameters.Add("@ID", txtAcc.Text);

                        cmd.ExecuteNonQuery();
                        sqlCommandDeleteStatus.ExecuteNonQuery();
